(ns general-sales.actor
  "GeneralSalesActor — the ISCO-08 5249 independent general-sales
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one sell/fulfill
  operation request (intake → advise → govern → decide → commit/hold,
  with a human-approval interrupt for escalated proposals). No
  infinite internal loop; checkpointed per superstep so an
  ...
  The unconditional invariant: the SalesAdvisor can never directly
  commit a record or dispatch a robot action the GeneralSalesGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [general-sales.advisor :as advisor]
            [general-sales.governor :as governor]
            [general-sales.store :as store]))

(defn build-graph
  "Build a compiled GeneralSalesActor graph. `store` implements
  `general-sales.store/Store`. `advisor` implements
  `general-sales.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:sale-id (:sale-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
